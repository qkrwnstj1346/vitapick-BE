...
import com.vita.vitapickBack.products.prd.PrdRepository; // ← 실제 경로에 맞게 수정
...
public class CusServiceImpl implements CusService {
	...
	private final PrdRepository prdRepository;       // ← 실제 경로에 맞게 수정
	...
	private String buildPrompt(String ansJson) {
		try {
			...
			return """
					...
					[우리 쇼핑몰 상품 목록]
					아래 상품 중에서만 추천하세요. (prd_id | 상품명 | 카테고리)
					%s
					...
					""".formatted(
					gender, ageGroup, height, weight, activity,
					status,    etcSuffix(statusEtc),
					diseases,  etcSuffix(diseasesEtc),
					meds,      etcSuffix(medsEtc),
					curSupp,   etcSuffix(curSuppEtc),
					allergy,   etcSuffix(allergyEtc),
					effects,   etcSuffix(effectsEtc),
					interested, etcSuffix(interestedEtc),
					prdContext
			);
	...
	// ─── [내부] prd 테이블 → 상품 목록 컨텍스트 문자열 생성 ──────────────────
	private String buildPrdContext() {
		try {
			// ※ prd 엔티티 필드명(getPrdId, getPrdName, getCatName)은
			//   실제 Prd 엔티티에 맞게 수정하세요.
			return prdRepository.findAll().stream()
					.map(prd -> "%d | %s | %s".formatted(
							prd.getPrdId(),
							prd.getPrdNm(),
							prd.getCatCd() != null ? prd.getCatCd() : ""))
					.collect(Collectors.joining("\n"));
		} catch (Exception e) {
			log.warn("** [CUS] 상품 목록 로드 실패 — 빈 컨텍스트로 진행: {}", e.getMessage());
	...
	// ─── [내부] prd 상세 조회 후 CusItDTO 빌드 ───────────────────────────────
	private CusItDTO fetchPrdDetail(Long cusItId, Long prdId, int sortNum) {
		CusItDTO dto = CusItDTO.builder()
				.cusItId(cusItId)
				.prdId(prdId)
				.sortNum(sortNum)
				.build();
		try {
			// ※ prd 엔티티 필드명(getPrdName, getPrdImg, getPrdPrice)은
			//   실제 Prd 엔티티에 맞게 수정하세요.
			prdRepository.findById(prdId).ifPresent(prd -> {
				dto.setPrdName(prd.getPrdNm());
				//dto.setPrdImg(prd.getPr());
				//dto.setPrdPrice(prd.getPrdPrice());
			});
		} catch (Exception e) {
